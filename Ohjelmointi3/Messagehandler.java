...
public class Messagehandler implements HttpHandler {
    ...
    private void handlejson(HttpExchange t) throws IOException, SQLException {
        ...
        if (newUser == null || newUser.length() == 0) {
            ...
            else if(!(obj.getString("dangertype").equals("Deer") || obj.getString("dangertype").equals("Moose")
    ...
    private void handlequery() throws IOException, SQLException {
        JSONArray array;
        if(obj.getString("query").equals("user")) {
            array = db.userQuery(obj);
            response = array.toString();
            code = 200;

        } else if (obj.getString("query").equals("time")){
            array = db.timeQuery(obj);
            response = array.toString();
            code = 200;
        } else {
            code = 413;
            response = "invalid user and or time";
        }
